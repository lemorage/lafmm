...
│       ├── group.toml
│       └── {TICKER}/{YEAR}.csv
├── profile.md                  # who the human is (experience, risk, goals, biases, system)
├── accounts/                   # trading accounts — one folder per account
│   └── {account-name}/
│       ├── account.toml        # broker, type, instruments, fees
│       └── journal/            # trade logs + observations, year-partitioned
│           └── {YEAR}/{MM-DD}.md
├── .claude/
...
- **Account config**: `cat accounts/ibkr-margin/account.toml`
- **Recent trades**: `ls accounts/ibkr-margin/journal/2026/`
- **Specific day**: `cat accounts/ibkr-margin/journal/2026/03-15.md`

`cache/` is regenerated by the sync skill (`.claude/skills/sync-lafmm-cache/`). Never edit it directly. When `data/` changes, run the sync skill to rebuild `cache/`.
...
- **New account** — create a new folder in `accounts/` with `account.toml` and `journal/`.
